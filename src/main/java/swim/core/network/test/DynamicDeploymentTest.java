...
import swim.core.network.test.NetworkLifeTimeTest.Result;

/**
 * @author Saleh Ibrahim, Sherif Tolba
 *
 */
public class DynamicDeploymentTest {
	...
	static double Delta[] = {125, 500, 2000, 8000 ,32000};
	...
	//M = Max Number of Surface Gateway Nodes
	public DynamicDeploymentTest(UWSensorNetwork network, int N, int M){
	...
	public DynamicDeploymentTest(UWSensorNetwork network){
		this.network = network;
		this.M = 5;
		this.N = 5;
		summaryResult = new Result[M];
...
//		gatewayType.setTransportEnergyRate(10);
	...
	Problem createProblem(int seed, int T) throws InvalidNodeId, InvalidLinkId{
		...
		{//Generate fixed under-water nodes
			...
			network.deferredNodeInit((int)Math.pow(N,2)+M+25, (int)Math.pow(N,2)+M+25);
			...
			for(Integer i : p.fixedNodes){
				network.setResidualEnergy(i.intValue(), 1e4);
			}
		}
		...
		for(int i = M; i <= M; i++){
			...
			for(int a = 0; a < Result.length && feasible; a++){
				...
				if(a == 1){
					...
					p.getObjective().addComponent(DOM.objectiveComponentId.NetworkLifeTime, Delta[a]/(localResult.o[0].doubleVal()-localResult.o[1].doubleVal()));
					p.getObjective().addComponent(DOM.objectiveComponentId.AverageDelay, (1-Delta[a])/(localResult.c[1].doubleVal()-localResult.c[0].doubleVal()));
	...
	public void test() throws Exception{
		...
		System.out.println("Dynamic Redployment with Network Lifetime Objective");
	...
	static class Result{
		...
		Result(){
			...
			length = Delta.length;			//TODO: check usage purpose and correct if necessary 
											// (source: Length = sizeof(Delta)/sizeof(double)) 
