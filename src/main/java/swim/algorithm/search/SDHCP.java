 ...
 */
public class SDHCP extends SearchAlgorithm {
    ...
    private final float HEX_SIDE_LEN,
                        DIVERGENCE_STEP_LEN,
                        NUM_DIVERGENCE_STEP_MULTIPLES = 7,
    ...
    private float travelDistOne, travelDistTwo, travelDistThree;
    ...
    public SDHCP(UWVehicleControl vehControl, Simulator sim) {
    ...
    protected void search() {
        ...
        if(!turnSeqSet) {
            ...
            for(int i = 1; i <= NUM_DIVERGENCE_STEP_MULTIPLES; i++) {
                ...
                travelDistThree = (i * DIVERGENCE_STEP_LEN) - (2 * vehControl.getActiveTurnRadius() * FastMath.tan(FastMath.PI/6));

                for( int j = 1; j <= 7; ++j ) {   
                    compositeTurnSeq.add(new Pair<String,Object>("travel", travelDistThree));
                    if( j < 7 ) {
                        compositeTurnSeq.add(new Pair<String,Object>("turn_direction", turnDir));
